...
#         y0, final_states = self.rnn(packed_sequencey,packed_sequence)
#
#         if self.bidirectional:
#             h = self.dropout(torch.cat((final_states[0][0], final_states[0][1]), dim=-1))
#         else:
...
class GRUEncoder(nn.Module):
    def __init__(self, in_size, hidden_size, out_size, num_layers=1, dropout=0.2, bidirectional=False):
        super().__init__()
        self.in_size = in_size  # 目标输入特征维度
        self.hidden_size = hidden_size
        self.out_size = out_size
        self.num_layers = num_layers
        self.dropout = dropout
        self.bidirectional = bidirectional  # 将 bidirectional 保存为类的属性

        # GRU 定义
        self.gru = nn.GRU(
            in_size, hidden_size, num_layers=num_layers, dropout=dropout,
    ...
    def forward(self, x, mask):
        # 假设 x 的形状是 [batch_size, feature_size]
        # 我们需要将其转换为 [sequence_length, batch_size, feature_size]
        # 这里假设 sequence_length 是 1，因为 x 是二维的
        x = x.unsqueeze(0)  # 现在 x 的形状是 [1, batch_size, feature_size]

        # 根据 mask 计算 lengths
        # mask 的形状是 [batch_size, 1, 1]
        # 我们需要将其转换为 [batch_size]
        lengths = (~mask.squeeze(-1).squeeze(-1)).to(torch.int64)  # 形状: [batch_size]
        lengths = torch.clamp(lengths, min=1)  # 确保 lengths 不小于 1

        # 打包输入序列
        packed_sequence = pack_padded_sequence(x, lengths, enforce_sorted=False)

        # GRU 前向传播
        gru_output, final_states = self.gru(packed_sequence)

        # 解包输出序列
        unpacked_output, _ = pad_packed_sequence(gru_output)

        # 处理双向 GRU 的最终状态
        if self.bidirectional:
            final_states = torch.cat((final_states[-2], final_states[-1]), dim=-1)
        else:
            final_states = final_states[-1]

        # 应用 dropout 和线性层
        h = self.dropout_layer(final_states)
        y_1 = self.linear_1(h)

        # 使用 GRU 的最后一个输出
        last_output = self.dropout_layer(unpacked_output[-1])
        y_2 = self.linear_2(last_output)
